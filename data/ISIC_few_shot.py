...
class SubDataset_Crop:
    def __init__(self, sub_meta, cl, target_transform=identity, min_size = 50,
        size_crops=[224],
        nmb_crops=[2],
        min_scale_crops=[0.2],
        max_scale_crops=[0.5],
...
class SetDataset_Crop:
    def __init__(self, batch_size):
        ...
        sub_data_loader_params = dict(batch_size = batch_size,
                                  ...
                                  pin_memory = False)        
        for cl in self.cl_list:
            sub_dataset = SubDataset_Crop(self.sub_meta[cl], cl)
            self.sub_dataloader.append( torch.utils.data.DataLoader(sub_dataset, **sub_data_loader_params) )
...
class Eposide_DataManager():
    def __init__(self, n_way=5, n_support=5, n_query=15, n_eposide=100):        
    ...
    def get_data_loader(self): 
        dataset = SetDataset_Crop(self.batch_size)
        sampler = EpisodicBatchSampler(len(dataset), self.n_way, self.n_eposide)  
        data_loader_params = dict(batch_sampler=sampler, num_workers=12, pin_memory=True)   
        data_loader = torch.utils.data.DataLoader(dataset, **data_loader_params)
        return data_loader
...
if __name__ == '__main__':
    ...
    base_datamgr            = SetDataManager(224, n_query = 15)

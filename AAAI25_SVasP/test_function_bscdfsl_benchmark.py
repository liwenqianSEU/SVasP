...
def test_bestmodel_bscdfsl(acc_file, name, dataset, n_shot, save_epoch=-1):
  ...
  model = SVasPGNN(model_dict[params.model], params, **few_shot_params)
  model = model.cuda()
  model.eval()
  ...
  print('%s  %d test iterations: Acc = %4.2f%% +- %4.2f%%' %(dataset, real_iter_num, acc_mean, 1.96* acc_std/np.sqrt(real_iter_num)))
  print('%sshot %15s  %d test iterations: Acc = %4.2f%% +- %4.2f%%' %(n_shot, dataset, real_iter_num, acc_mean, 1.96* acc_std/np.sqrt(real_iter_num)), file = acc_file)

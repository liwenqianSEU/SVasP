...
class MetaTemplate(nn.Module):
  ...
  def train_loop(self, epoch, train_loader_ori,  optimizer, model, total_it):
    print_freq = len(train_loader_ori) // 10
    avg_loss=0
    for i, (x, global_y ) in enumerate(train_loader_ori):
      ...
      # epsilon_list = [0.4, 0.04, 0.004]
      epsilon_list = [0.8, 0.08, 0.008]

      scores_fsl_ori, scores_fsl_adv, scores_cls_ori, scores_cls_adv, loss_fsl_ori, loss_cls_ori, loss_fsl_adv, loss_cls_adv, loss_domain, loss_cls_crop \
          = self.set_forward_loss_SVasP(x, global_y, epsilon_list, epoch)

      # consistency loss between initial and SVasP
      ...
      # final loss 
      k1, k2, k3, k4, k5, k6, k7, k8 = 1, 1, 1, 1, 0, 0, 1, 1     
      loss = k1 * loss_fsl_ori + k2 * loss_fsl_adv + k3 * loss_fsl_KL + k4 * loss_cls_ori + k5 * loss_cls_adv + k6 * loss_cls_KL + k7 * loss_domain + k8 * loss_cls_crop
      loss.backward()
      optimizer.step()
      ...
      if (total_it + 1) % 10 == 0 and self.tf_writer is not None:
        ...
        self.tf_writer.add_scalar('loss_cls_Kl:', loss_cls_KL, total_it +1)
        self.tf_writer.add_scalar('loss_domain:', loss_domain.item(), total_it +1)
        self.tf_writer.add_scalar('loss_cls_crop:', loss_cls_crop.item(), total_it +1)
